    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form id="edit-species-form" action="{{ route('admin.species.update', $species) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div>
                                ...
                                <select name="taxon_group" id="taxon_group" 
                                    ...
                                    @foreach($taxonGroups as $group)
                                        <option value="{{ $group }}" @selected(old('taxon_group', $species->taxon_group) == $group)>{{ $group }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="iucn_category" id="iucn_category"
                                    ...
                                    @foreach($iucnCategories as $code => $label)
                                        <option value="{{ $code }}" @selected(old('iucn_category', $species->iucn_category) == $code)>{{ $code }} - {{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="boe_status" id="boe_status"
                                    ...
                                    @foreach($boeStatuses as $key => $label)
                                        <option value="{{ $key }}" @selected(old('boe_status', $species->boe_status) == $key)>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="ccaa_status" id="ccaa_status"
                                    ...
                                    @foreach($ccaaStatuses as $key => $label)
                                        <option value="{{ $key }}" @selected(old('ccaa_status', $species->ccaa_status) == $key)>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="cites_appendix" id="cites_appendix"
                                    ...
                                    @foreach($citesAppendices as $code => $label)
                                        <option value="{{ $code }}" @selected(old('cites_appendix', $species->cites_appendix) == $code)>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <div class="mt-6 flex justify-between">
                        <form action="{{ route('admin.species.sync', $species) }}" method="POST">
                            @csrf
                            <button type="submit"
                        ...
                        <div class="flex gap-3">
                            ...
                            <button type="submit" form="edit-species-form"
