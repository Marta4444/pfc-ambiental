<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- Aviso si el caso está finalizado --}}
            @if($report->isFinalizado())
                <div class="mb-4 bg-gray-100 border-l-4 border-gray-500 text-gray-700 p-4 rounded-lg flex items-center">
            ...
            {{-- Aviso si no puede editar por otras razones --}}
            @elseif(!$canEdit)
            ...
            @if($groupedDetails->isEmpty())
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-center">
                        ...
                        @if($canEdit)
                            <div class="mt-6">
                                <a href="{{ route('report-details.create', $report) }}" class="inline-flex items-center px-4 py-2 bg-eco-600 text-white rounded-md hover:bg-eco-700">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                                    </svg>
                                    Añadir Detalle
                                    ...
                                    @foreach($details as $detail)
                                        <div class="bg-gray-50 rounded p-3">
                                            <p class="text-xs font-medium text-gray-500 uppercase">
                                                {{ $fields->get($detail->field_key)?->label ?? ucfirst(str_replace('_', ' ', $detail->field_key)) }}
                                            </p>
