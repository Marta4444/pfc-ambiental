...
class SpeciesSyncService
{
    ...
    public function syncSpecies(Species $species): bool
    {
        ...
        try {
            ...
            if ($this->citesToken) {
                ...
                Log::warning('CITES token not configured, skipping CITES enrichment.');
            }
    ...
    private function fetchFromIucn(string $scientificName): ?array
    {
        ...
        // Only return cached result if it was previously successful
    ...
    private function fetchFromCites(string $scientificName): ?array
    {
        ...
        $cacheKey = 'cites_v2_' . md5($scientificName);
        ...
        try {
            ...
            if ($response->successful()) {
                $data = $response->json();
                $result = $data['taxon_concepts'][0] ?? null;
                if ($result !== null) {
                    Cache::put($cacheKey, $result, 86400);
                }
                return $result;
            ...
            Log::warning("CITES API non-success for {$scientificName}", [
