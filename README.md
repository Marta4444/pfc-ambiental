...
6. Hacer cambios en el código de desarrollo para adaptar a producción - quitar seeders de datos falsos de reports y demás.
7. Hacer push a github y esto lanza un nuevo deployment.
...
10. Para sincronizar especies y areas protegidas, se hace entrando como admin, desde el front end. Hay botones para sincronizar, porque desde el cliente me daba error y era la única forma.. 
...
3. La app se despliega sin estilos porque hay un problema con el css. Railway es un proxi inverso. Laravel ignora por defecto el header X-Forwarded-Proto: https que Railway envía, generando las URLs del CSS/JS con http://. El navegador las bloquea por mixed content. Solución: Añadir $middleware->trustProxies(at: '*') en app.php